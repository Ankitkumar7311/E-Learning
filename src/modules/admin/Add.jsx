      ...
      <tbody className="text-gray-700">
        {students && students.length > 0 ? (
          students.map((student, idx) => (
            ...
            >
              <td className="px-4 py-2 text-center border-r-2 border-white whitespace-nowrap">
                {student.name}
              </td>
              <td className="px-4 py-2 text-center border-r-2 border-white whitespace-nowrap">
                {student.studentId}
...
const Add = () => {
  ...
  // Auto-fetch name for removal
  useEffect(() => {
    const fetchStudentName = async () => {
      if (studentIdToRemove.trim()) {
        try {
          const response = await fetch(
            `${API_BASE_URL}/searchByStudentId/${studentIdToRemove}`
          );
          if (!response.ok) {
  ...
  const handleAddSubmit = async (e) => {
    ...
    if (Object.values(formData).some((field) => field === "")) {
      toast.error("Please fill out all fields to add a student.");
      return;
    }
    try {
      ...
      toast.error("Error adding student. Please check the console.");
    }
  };
  ...
  const handleRemoveSubmit = async (e) => {
    ...
    if (!studentIdToRemove) {
      toast.error("Please enter a Student ID to remove.");
      return;
    }
    try {
      const response = await fetch(
        `${API_BASE_URL}/deleteStudent/${studentIdToRemove}`,
        { method: "DELETE" }
      );
      if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
      ...
      toast.error("Error removing student. Please check the console.");
    }
  };
        ...
        >
          ...
          <div className="flex flex-col gap-y-5">
            ...
            <InputField
              ...
              required
            />
            <ActionButton type="submit" variant="add">
              Add Student
            </ActionButton>
          </div>
        </form>
        ...
        >
          ...
          <div className="flex flex-col gap-y-5">
            ...
            <InputField
              ...
              disabled
            />
            <ActionButton type="submit" variant="remove">
              Remove Student
            </ActionButton>
          </div>
        </form>
