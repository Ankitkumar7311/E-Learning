...
const getStoredStudentId = () => {
  try {
    ...
    if (vs) {
      const parsed = JSON.parse(vs || "{}");
      if (parsed?.studentId) {
        console.log("📦 [STORAGE] Found studentId in vidyaSarthiAuth:", parsed.studentId);
        return parsed.studentId;
      }
      if (parsed?.user?.studentId) {
        console.log("📦 [STORAGE] Found studentId in vidyaSarthiAuth.user:", parsed.user.studentId);
        return parsed.user.studentId;
      }
    }
    const stored = localStorage.getItem("user");
    if (!stored) {
      console.log("📦 [STORAGE] No user data found in localStorage");
      return null;
    }
    const parsed = JSON.parse(stored);
    const id = parsed?.studentId || parsed?.userId || null;
    console.log("📦 [STORAGE] Found studentId in user:", id);
    return id;
  } catch (e) {
    console.warn("❌ [STORAGE] Failed to read studentId from localStorage", e);
    return null;
  }
};
...
const readToken = () => {
  try {
    ...
    if (vs) {
      const parsed = JSON.parse(vs || "{}");
      if (parsed?.token) {
        console.log("📦 [STORAGE] Found token in vidyaSarthiAuth");
        return parsed.token;
      }
    }
    const token = localStorage.getItem("token") || null;
    console.log("📦 [STORAGE] Token from localStorage:", token ? "Present" : "Missing");
    return token;
  } catch {
    return localStorage.getItem("token") || null;
  }
...
let SectionRight = () => {
  ...
  console.log("🚀 [INIT] Component loaded with studentId:", studentId);
  ...
  useEffect(() => {
    ...
    const fetchSemester = async () => {
      if (!studentId) {
        setSemesterError("No student ID");
        console.log("⚠️ [SEMESTER API] Skipped - No studentId");
        return;
      }
      setLoadingSemester(true);
      setSemesterError("");
      try {
        const url = `${API_BASE}/student/getCurrentSemester/${encodeURIComponent(studentId)}`;
        console.log("🔍 [SEMESTER API] Calling:", url);
        console.log("🔍 [SEMESTER API] Student ID:", studentId);
        console.log("🔍 [SEMESTER API] Token:", token ? "Present" : "Missing");
        
        const res = await fetch(url, { method: "GET", headers: token ? { Authorization: `Bearer ${token}` } : {} });
        console.log("🔍 [SEMESTER API] Response status:", res.status);
        
        if (!res.ok) throw new Error(`Status ${res.status}`);
        
        const text = await res.text();
        console.log("🔍 [SEMESTER API] Response text:", text);
        const sem = parseInt(text.trim(), 10);
        ...
        if (!cancelled) {
          if (Number.isNaN(sem)) {
            setSemesterError("Not available");
            setCurrentSemester(null);
            console.log("⚠️ [SEMESTER API] Invalid semester value");
          } else {
            setCurrentSemester(sem);
            setSemesterError("");
            console.log("✅ [SEMESTER API] Success - Semester:", sem);
          }
        }
      } catch (err) {
        if (!cancelled) {
  ...
  const fetchProfileStatus = async () => {
    if (!studentId) {
      setProfileStatusError("No student ID");
      setProfileComplete(null);
      console.log("⚠️ [PROFILE API] Skipped - No studentId");
      return;
    }
    setProfileStatusLoading(true);
    setProfileStatusError("");
    try {
      const url = `${API_BASE}/student/checkStatus/${encodeURIComponent(studentId)}`;
      console.log("🔍 [PROFILE API] Calling:", url);
      console.log("🔍 [PROFILE API] Student ID:", studentId);
      console.log("🔍 [PROFILE API] Token:", token ? "Present" : "Missing");
      ...
      setProfileComplete(/profile\s*completed/i.test(text));
      console.log("✅ [PROFILE API] Success - Complete:", /profile\s*completed/i.test(text));
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchCount = async () => {
      ...
      try {
        const url = `${API_BASE}/student/getTotalMaterial`;
        console.log("🔍 [MATERIAL API] Calling:", url);
        console.log("🔍 [MATERIAL API] Token:", token ? "Present" : "Missing");
        
        const res = await fetch(url, { method: "GET", headers: token ? { Authorization: `Bearer ${token}` } : {} });
        console.log("🔍 [MATERIAL API] Response status:", res.status);
        
        if (!res.ok) throw new Error(`Status ${res.status}`);

        const text = await res.text();
        console.log("🔍 [MATERIAL API] Response text:", text);
        const cnt = parseInt(text.trim(), 10);
        ...
        if (!cancelled) {
          if (Number.isNaN(cnt)) {
            setMaterialCountError("N/A");
            setMaterialCount(null);
            console.log("⚠️ [MATERIAL API] Invalid count value");
          } else {
            setMaterialCount(cnt);
            setMaterialCountError("");
            console.log("✅ [MATERIAL API] Success - Count:", cnt);
          }
        }
      } catch (err) {
        if (!cancelled) {
