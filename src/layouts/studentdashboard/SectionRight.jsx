...
import { Megaphone, FileStack, FileQuestion, NotebookPen, MessageSquare, Icon, CloudLightningIcon, StarIcon, School, StarHalfIcon } from 'lucide-react';
...
import FacultyFeedback from "../../modules/student/find/FacultyFeedback";
import FinalFeedback from "../../modules/student/find/FinalFeedback";
...
// ... (getStoredStudentId and readToken helpers remain the same) ...
...
let SectionRight = () => {
  ...
  useEffect(() => {
    ...
    const fetchSemester = async () => {
      ...
      try {
        ...
        const sem = parseInt(text.trim(), 10); // This should be a number e.g. 7 or 8
        ...
        if (!cancelled) {
          if (Number.isNaN(sem)) {
            ...
            setCurrentSemester(sem); // Set the number here
            setSemesterError("");
          }
        }
      } catch (err) {
  ...
  // ... (fetchProfileStatus and material count useEffects remain the same) ...
  ...
  const features = [
    ...
    { key: "complains", p: "Send Request" },
    { key: "materials", p: "Uploaded Material" },
  ];

  // --- DYNAMIC UPLOADS ARRAY ---
  const uploads = useMemo(() => {
    // This is the base list of uploads available to everyone
    const baseUploads = [
      ...
      { title: "Faculty Feedback", tag: "Share feedback with faculty", component: <FacultyFeedback />, icon: <MessageSquare className="w-7 h-7" />, color: "bg-green-100 text-green-700", hoverColor: "hover:border-green-300 hover:bg-green-50" },
    ];

    // --- CONDITIONAL LOGIC ---
    // Check if the current semester is 7 or 8
    if (currentSemester === 7 || currentSemester === 8) {
      // If it is, add the "Final Feedback" option to the list
      baseUploads.push({
        title: "Final Feedback",
        tag: "Share feedback with college",
        component: <FinalFeedback />,
        icon: <StarIcon className="w-7 h-7" />,
        color: 'bg-red-200 text-red-700', // Changed text-red-100 to text-red-700 for better contrast
    ...
    return baseUploads;
  }, [currentSemester]); // This array will only recalculate when currentSemester changes
    ...
    <>
      <div className="w-full max-w-4xl mx-auto p-4 md:p-6">
          ...
          {features.map((item) => {
            const content = (
              <>
                ...
                {item.key === "complains" && (
                  <div className="flex flex-col items-center" onClick={() => onFeatureClick(item.p)}>
            ...
            const isClickable = item.key === 'complains' || item.key === 'profile';
            const clickHandler = () => {
              if (item.key === 'complains') onFeatureClick(item.p);
              if (item.key === 'profile') fetchProfileStatus();
            };
            
            return (
              <div 
                key={item.key} 
                className={`flex flex-col items-center space-y-2 min-w-0 ${isClickable ? 'cursor-pointer' : 'cursor-default'}`} 
        ...
        <section id="section2" className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {/* This 'uploads' array is now dynamic and will only show Final Feedback for sem 7/8 */}
