// src/modules/student/FindNotes.jsx
import { Download } from "lucide-react"; // Added import
 ...
 * - when a subject is chosen, calls /getMaterialListNOTES/{subjectCode} to get NOTES list
...
const BRANCHES = ["CSE", "CIVIL", "EEE", "ECE", "ME"]; // extend if needed
...
const FindNotes = () => {
  ...
  // fetch notes when subject selected
  useEffect(() => {
    ...
    const fetchMaterials = async () => {
      ...
      setMaterials([]);
      try {
        const url = `${API_BASE}/getMaterialListNOTES/${encodeURIComponent(selectedSubjectCode)}`;
        ...
        if (!cancelled) setErrorMaterials(err.message || "Error loading notes list");
      } finally {
      ...
      <section className="w-full max-w-3xl flex flex-col bg-white p-6 ">
        ...
        <div className="mb-4">
          ...
          {!loadingMaterials && materials.length === 0 && !errorMaterials && selectedSubjectCode && (
            <p className="text-sm text-gray-500">No notes found for the selected subject.</p>
          )}
          {!loadingMaterials && materials.length === 0 && !errorMaterials && !selectedSubjectCode && (
             <p className="text-sm text-gray-500">Please select a subject to see the notes list.</p>
          )}

          {materials.length > 0 && (
            // --- Added rounded border and border container ---
                  ...
                  {materials.map((m) => (
                    <tr key={m.materialId || m.id} className="hover:bg-gray-50">
                      ...
                      <td className="py-3 px-3 align-middle">
                        {/* --- Buttons with consistent padding/focus --- */}
