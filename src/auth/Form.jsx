...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    ...
    setLoading(true);
    try {
      // login returns the nextState with user/role (role is now normalized to 'teacher')
      const finalAuthState = await login({ email: data.email, password: data.password });

      // Note: AuthRedirect wrapper handles the actual immediate navigation.
      // This internal logic serves as a fallback/direct route for this specific form.
      ...
      } else if (role === 'teacher') { // Check only for normalized 'teacher'
  ...
  return (
    ...
    >
      ...
      <div
        className="relative z-10 w-full max-w-md sm:max-w-sm lg:max-w-md xl:max-w-lg rounded-2xl pt-6 px-4 sm:p-5 md:p-6 bg-white bg-opacity-80 shadow-lg"
        style={{
          backgroundImage: `url(${cardpic})`,
          backgroundSize: 'cover',
          backgroundPosition: 'center',
      ...
      >
        <h1 className="text-xl md:text-3xl font-medium mb-2 text-center">LogIn</h1>
        <hr className="border border-black mb-2 w-2/3 mx-auto" />
        <h4 className="text-xs md:text-lg mb-5 text-center">Welcome onboard with us!</h4>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3">
          ...
          <div className="mt-3 space-y-1">
            <p className="text-xs md:text-base text-center">
              Are you a Student?{' '}
              <b className="hover:underline cursor-pointer" onClick={() => navigate('/student-login')}>LogIn as a Student</b>
            </p>
            <p className="text-xs md:text-base text-center">
              Are you a Teacher?{' '}
              <b className="hover:underline cursor-pointer" onClick={() => navigate('/teacher-login')}>LogIn as a Teacher</b>
            </p>
          </div>
        </form>
      </div>
