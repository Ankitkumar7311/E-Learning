...
const FindPYQ = () => {
    ...
    const fetchRegs = async () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          // unknown shape -> try to normalize object
  ...
  useEffect(() => {
    ...
    const fetchSubjects = async () => {
      ...
      try {
        ...
        // same normalization as UploadNotes
  ...
  // fetch materials when subject selected
  useEffect(() => {
    ...
    const fetchMaterials = async () => {
      ...
      setMaterials([]);
      try {
        const url = `${API_BASE}/getMaterialListPYQ/${encodeURIComponent(selectedSubjectCode)}`;
        ...
        if (!res.ok) {
          ...
          throw new Error(t || `Failed to fetch PYQs (${res.status})`);
        }
        ...
        if (!cancelled) setErrorMaterials(err.message || "Error loading PYQ list");
      } finally {
  ...
  return (
    <div className="flex flex-col gap-10 justify-center items-center px-4 py-6">
      <section className="w-full max-w-3xl flex flex-col bg-white p-6 rounded-xl shadow">
        <h2 className="text-2xl font-bold text-center mb-6">Find PYQs</h2>
