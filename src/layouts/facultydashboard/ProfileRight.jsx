...
// Nicer completed/incomplete badges
const CompletedBadge = ({ className = "" }) => (
  <div className={`rounded-full w-12 h-12 flex items-center justify-center shadow-sm ${className}`}>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className="w-7 h-7" fill="none" aria-hidden>
      <circle cx="12" cy="12" r="12" fill="#10B981" /> {/* green circle */}
...
const ProfileRight = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfileStatus = async () => {
      try {
        ...
        if (ct.includes('application/json')) {
          const json = await res.json();
          if (json === true || json.completed === true || json.profileCompleted === true || json.status === 'COMPLETED' || (typeof json.message === 'string' && json.message.includes('Profile Completed'))) {
            setProfileStatus(true);
          } else {
            setProfileStatus(false);
          }
  ...
  return (
    <>
      ...
      <div className="w-full max-w-4xl mx-auto bg-white shadow-md rounded-xl p-4 md:p-8">
        
        {/* Features */}
        {/* RESPONSIVE CHANGE: Layout changes from a 2x2 grid on mobile to a 1x4 flex row on medium screens and up */}
        <section className="bg-white shadow-md rounded-2xl p-4 grid grid-cols-2 gap-4 md:flex md:h-32 md:justify-around md:items-center">
          {features.map((item, index) => (
            <div
              key={index}
              // RESPONSIVE CHANGE: Removed w-1/4 to allow grid/flex to control width
            ...
            >
              <div className="bg-blue-100 rounded-full flex items-center justify-center w-14 h-14">
                {item.type === 'count' ? (
                  ...
                  item.content === null ? (
                     <div className="w-12 h-12 rounded-full bg-gray-200 animate-pulse" /> // Loading skeleton
                  ) : item.content ? (
                    <CompletedBadge />
                  ) : (
                    <IncompleteBadge />
        ...
        {/* Upload Section */}
        {/* RESPONSIVE CHANGE: Layout changes from a single column on mobile to a two-column grid on small screens and up */}
