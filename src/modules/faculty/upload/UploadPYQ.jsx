// src/modules/faculty/upload/UploadPYQ.jsx
import React, { useCallback, useEffect, useMemo, useRef, useState } from "react";

// Inline icons (same as UploadNotes)
...
// helpers to read auth from localStorage
...
// unique id generator
const generateUniqueId = () => `pyq-${Date.now()}-${Math.random().toString(36).slice(2,9)}`;
...
const UploadPYQ = () => {
  ...
  // fetch subjects when regulation+branch+semester are selected
  useEffect(() => {
  ...
  // reset file inputs helper
  const resetFileInputs = useCallback(() => {
    fileInputRefs.current.forEach(input => { try { input.value = ""; } catch (_) { } });
  }, []);
  ...
  // resolve facultyId (prefer stored, else try backend using email in localStorage)
  ...
  // submission handler: upload per-row to /faculty/addNewPYQ
  const handleSubmit = useCallback(async (e) => {
    ...
    for (const r of uploadRows) {
      ...
      try {
        const resp = await fetch(`${API_BASE}/faculty/addNewPYQ`, {
          method: "POST",
          headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) },
          body: fd,
        });
    ...
    if (successCount > 0 && failed.length === 0) {
      setSubmission({ open: true, message: `Successfully uploaded ${successCount} PYQs!`, isError: false });
      setFormData(prev => ({ ...prev, subjects: Array(rowsCount).fill(""), files: Array(rowsCount).fill(null) }));
      resetFileInputs();
      ...
      setSubmission({ open: true, message: "All PYQ uploads failed. Check server logs.", isError: true });
        ...
        <div className="flex justify-center pt-4">
          <button
            ...
            className={`font-extrabold text-white px-12 py-3 rounded-full shadow-lg transition duration-300 ease-in-out w-full md:w-auto
              ${isUploading || !formData.selectedRegulation ? "bg-gray-400 cursor-not-allowed" : "bg-red-600 hover:bg-red-700 hover:shadow-xl transform hover:scale-105"}`}
          >
            {isUploading ? (
