...
export default function AuthRedirect() {
  ...
  useEffect(() => {
    // 2. If still loading, do nothing. Wait for auth to be resolved.
    if (loading) {
    ...
    // 3. If NOT authenticated (and no longer loading), redirect to login.
    if (!isAuthenticated || !role) {
      console.log('❌ Not authenticated, redirecting to login');
      navigate('/login', { replace: true });
      return;
  ...
  // 5. Add `loading` to the dependency array
  }, [isAuthenticated, role, user, navigate, loading]);

  // This spinner is shown while loading OR during the brief moment before navigation.
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100">
      <div className="text-center">
        <div className="animate-spin rounded-full h-16 w-16 border-b-4 border-indigo-600 mx-auto"></div>
