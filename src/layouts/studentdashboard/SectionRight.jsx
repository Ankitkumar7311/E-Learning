...
let SectionRight = () => {
  ...
  return (
    <>
      <div className="w-full max-w-4xl mx-auto bg-white shadow-md rounded-xl p-4 sm:p-8">
        {/* RESPONSIVE CHANGE: Changed to a 2x2 grid on mobile, and a horizontal flex layout on medium screens and up. */}
        <section id="section1" className="w-full bg-white shadow-md rounded-2xl grid grid-cols-2 md:flex md:justify-around items-start p-4 gap-4">
          {features.map((item) => {
            const content = (
              <>
                {item.key === "semester" && (
                ...
                {item.key === "profile" && (
                  <div className="flex flex-col items-center cursor-pointer">
                    <div className="rounded-full flex items-center justify-center w-14 h-14 bg-blue-100">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center shadow-sm ${profileComplete ? "bg-amber-500" : "bg-gray-100"}`}>
                        {profileStatusLoading ? (
            ...
            if (item.route) {
              return (
                <Link key={item.key} to={item.route} className={commonClasses}>
                  {content}
                </Link>
              );
            }
        ...
        {/* This section's responsive classes were already good. No changes needed. */}
        <section id="section2" className="w-full rounded-xl grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mt-4">
          {uploads.map((item, index) => (
            <div
              key={index}
              className="bg-white rounded-xl shadow-md min-h-[140px] flex flex-col items-center justify-between p-2 min-w-0 cursor-pointer hover:shadow-lg transition"
              onClick={() => setModalContent(item.component)}
            >
              <div className="bg-blue-100 rounded-lg w-full h-full flex flex-col items-center justify-center text-center p-2">
                <p className="text-sm sm:text-base font-bold bg-gradient-to-r from-[#1461FF] to-[#1B2767] bg-clip-text text-transparent break-words">
