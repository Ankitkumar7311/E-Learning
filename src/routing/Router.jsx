...
// import AdminDashboard from "../layouts/AdminDashboard";
// import AdminHome from "../modules/admin/AdminHome"; // <-- 1. IMPORTED ADMIN HOME
// import HomePage from "../pages/HomePage";
// import Courses from "../pages/Courses";
// import CentralLibrary from "../pages/CentralLibrary";
// import Facilities from "../pages/Facilities";
...
//         element: <AdminDashboard />,
//         children: [
//           // --- 2. ADDED INDEX ROUTE FOR THE DASHBOARD HOME ---
...
//       // --- 1. MOVED THIS ROUTE HERE TO PROTECT IT ---
//       { path: "update-profile-student", element: <UpdateStudent /> },
...
//   // --- 2. REMOVED INSECURE/REDUNDANT ROUTES ---
//   // { path: "update-profile-student", element: <UpdateStudent /> }, // <-- MOVED
//   // { path: "/profile", element: <Profile /> }, // <-- REDUNDANT
//   // { path: "/update-teach", element: <UpdateTeach /> }, // <-- REDUNDANT
//   // { path: "/edit-material", element: <EditMaterial /> }, // <-- REDUNDANT

//   { path:"help-support",element: <HelpAndSupportForm/>}, // <-- This is public (OK)
...
const ProtectedRoute = ({ children, role }) => {
  ...
  useEffect(() => {
    console.log("- ROUTE_DEBUG: ProtectedRoute useEffect triggered. State:", { loading, isAuthenticated });

    if (loading) {
        console.log("- ROUTE_DEBUG: Auth is loading. Waiting...");
        return;
    }

    if (!isAuthenticated) {
        console.log("- ROUTE_DEBUG: User is not authenticated. Not logging visit.");
        return;
    }
    
    if (visitLoggedRef.current) {
        console.log("- ROUTE_DEBUG: Visit already logged for this session. Skipping.");
        return;
    }

    // If we reach here, user is authenticated and visit has not been logged.
    console.log("✅ ROUTE_DEBUG: Conditions met. Attempting to log visit...");
    fetch(`${API_BASE_URL}/log-visit`, { method: 'POST' })
      .then(response => {
        if (response.ok) {
          console.log("✅ ROUTE_DEBUG: API call successful. Visit logged.");
          visitLoggedRef.current = true;
        } else {
          console.error(`❌ ROUTE_DEBUG: API call failed with status: ${response.status}`);
        }
      })
      .catch(err => {
        console.error("❌ ROUTE_DEBUG: Network error during visit log:", err);
      });
