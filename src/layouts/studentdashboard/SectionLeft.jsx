...
// const SectionLeft = ({ onRequestEdit }) => {
//   return (
//     <div className="h-full w-full md:w-1/2 flex justify-center py-3 shadow-sm">
//       <div className="flex flex-col rounded-xl bg-white h-auto md:h-[525px] w-[90%] sm:w-72 md:w-60 p-3">
//         <div className="flex flex-col items-center gap-2">
...
const SectionLeft = ({ onRequestEdit }) => {
  ...
  useEffect(() => {
    const fetchStudent = async () => {
      setLoading(true);
      try {
        const response = await fetch(`http://localhost:8080/searchByStudentId/{studentId}`
        );
    ...
    <div className="h-full w-full  md:w-1/2 flex justify-center py-3 shadow-sm">
      <div className="flex flex-col rounded-xl bg-white h-auto md:h-[525px] w-[90%] sm:w-72 md:w-60 p-3">
        <div className="flex flex-col items-center gap-2">
          ...
          />
          <h4 className="text-center text-sm sm:text-base md:text-lg">
            {loading ? "Loading..." : studentData?.name || "PREM KUMAR"}
          </h4>
        </div>
        ...
        <span className="text-[13px] sm:text-[15px]/[20px]">
          {/* Subject: {studentData?.subject || "Compiler Design"} <br /> */}
          Regulation: {studentData?.regulation || "2020 - 24"} <br />
          studentId: {studentData?.roll || "200XXXX"}
        </span>
        ...
        <span className="text-sm sm:text-base">
          Phone: <br />
          {studentData?.phone || "+91 XXXXXXXXXX"} <br />
          Email : <br />
          {studentData?.email || user?.user || "XXXXXXXXXXXXXX"}
        </span>
        ...
        <span className="text-sm sm:text-base">
          Address: <br />
          {studentData?.address || "Village, Block, Lakhisarai"} <br />
          Bihar 811315 <br />
        </span>
